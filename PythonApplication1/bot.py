...
def send_next_photo(bot, update):
    ...
    user = debe.get_user(update.message.chat_id)

    if user_choice[0] == variants[0][0]:
        bot.sendPhoto(update.message.chat_id, photo=photos[i], reply_markup=rm)
        debe.add_view(user,i, -1)
        return SEND_NEXT_PHOTO
    else:
        bot.sendPhoto(update.message.chat_id, photo=photos[i], reply_markup=rm)
        debe.add_view(user,i, 1)
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            SEND_NEXT_PHOTO: [MessageHandler([Filters.text], send_next_photo), CommandHandler('save', save)],
